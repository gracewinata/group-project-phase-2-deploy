...
export default function MultiPlayPage({ base_url }) {
  ...
  function nextSequence() {
    ...
    /** We dont broadcast gamePattern bcz react updates the state asynchronously */
    const newSequence = [...gamePattern, randomChosenColour];
    setGamePattern(newSequence);

    /** this is to broadcast the new sequence to all players in the room ( tapi lewat server dulu ) */
    socket.current.emit("sequenceGenerated", { roomName, sequence: newSequence });

    /* Animasi suara dan flash tombol */
  ...
  async function getTips() {
    try {
      const { data } = await axios.get(`${base_url}/tips`, {
        headers: {
  ...
  function checkAnswer(currentLevel, updatedPattern) {
    ...
    if (pattern[currentLevel] === gamePattern[currentLevel]) {
      ...
      /** This is to tell the sserver that this player has lost */
      socket.current.emit("gameOver", { roomName, score: level });
    }
  }
  ...
  async function recordHistory() {
    try {
      ...
      const { data } = await axios.post(`${base_url}/game`, body, {
        headers: {
          Authorization: `Bearer ${localStorage.access_token}`,
        },
      });
  ...
  useEffect(() => {
    /** Inisialisasi koneksi socket pas user buka page */
    socket.current = io(`${base_url}`);
    socket.current.connect();

    /** Ngasih tau server kalo user join room ini, dan nunggu response konfirmasi dari server */
    socket.current.emit("joinRoom", roomName);
    socket.current.on("roomJoined", (message) => {
      console.log(`Server response: ${message}`);
    });

    /** Listening for new sequence */
    socket.current.on("sequenceGenerated", ({ sequence }) => {
      console.log("New sequence received:", sequence);
      setGamePattern(sequence); // Update game pattern for other client that didnt trigger the event
    });

    /** Clean up socket pas proses unmounting */
    return () => {
      if (socket.current) {
        socket.current.off("sequenceGenerated");
